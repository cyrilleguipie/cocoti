...
function validateSteps() {
    $('#placement-form').validator().on('submit', function (e) {
...
/*    if ($('#placement-form').length) {
        $('#placement-form').validator();
    }*/
    ...
    $('.step-trigger-btn').on('click', function () {
        ...
        var currentStepBtn = $(this).attr('href');
        var currentStepInputs = currentStep.find("input,select");
        var isValid = true;
        ...
        if(currentStepInputs.length > 0){
            for(var i=0; i<currentStepInputs.length; i++){
                if (!currentStepInputs[i].validity.valid){
                    isValid = false;
                    $(currentStepInputs[i]).closest(".form-group").addClass("has-error has-danger");
                }
            }
        }

        if (isValid){
            $('.step-trigger[href="' + currentStepBtn + '"]').click();
        } else {
